...
public class DeleteNodeInBST450 {
    ...
    public TreeNode deleteNode(TreeNode root, int key) {
        ...
        if(root.val < key) root.right = deleteNode(root.right, key);
            ...
            // root.val == val
            if(root.left == null) return root.right; // 只有一个child
            else if(root.right == null) return root.left; // 只有一个child
            else { // 有两个children
                TreeNode newRoot = root.right;
                TreeNode parent = null;
                // 找出右子树中最左边的作为new root
                while(newRoot.left != null) {
                    parent = newRoot;
                    newRoot = newRoot.left;
                }
                if(parent == null) { // 注意检查parent是否为null，是的话说明root.right就是右边最小的
